...
| work is waiting in the base's queue (say *"process the work queue"*, or nod when Odin mentions it on load) | `queue` | Works the tasks you authored from another surface (e.g. the web control plane) — each task was consented at authoring, scoped to exactly what it names; results land as candidates for your review, and every task closes with an honest verdict: done, blocked, or declined. |
...
(`integrity.derived_self_hash` in your base's `muninn.yml` — **on by default for
newly created bases** since T-243, opt-in for bases created earlier) makes
...
2. If your base predates T-243, consider enabling integrity checking
   (`integrity.derived_self_hash: true`, see *Configuration* above) so any
   accidental out-of-band edit is flagged by the next `lint` instead of
   slipping in silently. Newly created bases have it on already.
