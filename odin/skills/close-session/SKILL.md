...
**Also check for stranded branches**, the failure mode with no other signal.
A branch ahead of the base's default branch holds work that was authored and
never arrived: lint cannot flag it (it verifies what is present), staleness
cannot (it needs a source to compare), and the web control plane cannot (it
reads only the default branch). Cloud sessions produce these by default, since
they push to their own working branch. Read them locally with
`git branch -r --no-merged origin/<default>` (no fetch; this reports what this
clone last saw), then for each one name it and **offer** the merge or a PR.
Say plainly what is stranded ("2 sources + 1 insight from Jul 18"), not just
the branch name. Never merge without the nod.
