...
from .util import _append_log, _dump_yaml, _load_yaml, _locked, _valid_id, atomic_rename, atomic_replace  # noqa: E402


#: T-233: full-tier captures at or past this many held bytes require an explicit
#: allow_large. Sources are immutable (§I2), so oversized blobs live in git
#: history forever, and hosts hard-fail pushes near 100 MB — the failure would
#: otherwise arrive at push time, far from the act that caused it.
LARGE_CAPTURE_BYTES = 25 * 1024 * 1024
...
def capture(root, id, body, *, origin, tier="full", capture_reason=None,
    ...
    return _capture(root, id, raw=body.encode("utf-8"), canonical_name="source.md",
                    ...
                    upstream_identity=upstream_identity, allow_large=allow_large)
...
def capture_file(root, id, raw, filename, *, origin, tier="full",
                 ...
                 upstream_identity=None, allow_large=False):
...
def _capture(root, id, *, raw, canonical_name, origin, tier, capture_reason,
    ...
    # T-233 size guard: holding the bytes is the deliberate act, so past the
    # threshold it must be said out loud. Refuse BEFORE any write, naming both
    # exits; reference tier is exempt — it holds no bytes.
    if tier == "full" and len(raw) >= LARGE_CAPTURE_BYTES and not allow_large:
        mb = len(raw) / (1024 * 1024)
        limit_mb = LARGE_CAPTURE_BYTES // (1024 * 1024)
        raise ValueError(
            f"holding {mb:.0f} MB of source bytes (threshold {limit_mb} MB). "
            "Sources are immutable, so held bytes live in git history forever, "
            "and hosts refuse pushes near 100 MB. Either capture "
            "tier=reference with a capture_reason (locator only, no bytes "
            "held), or pass allow_large (--allow-large) to hold them "
            "deliberately.")
    ...
    if sdir.exists():
        ...
        atomic_replace(tmp_canon, sdir / canonical_name)
        if tmp_aid is not None:
            atomic_replace(tmp_aid, sdir / aid_name)
        elif cur_aid and (sdir / cur_aid).exists():
            (sdir / cur_aid).unlink()          # new version is bytes-only
        atomic_replace(tmp_meta, meta_p)               # THE commit point
    ...
    atomic_rename(tmp, sdir)  # single atomic move into place
...
@_locked
def reextract(root, id=None, when=None):
    """T-226: backfill text aids from **held bytes** for sources whose format
    has an extractor NOW — the repair for a source captured bytes-only before
    its extractor existed (an identical-byte re-capture dedups to a no-op, so
    without this op there is no path to the aid short of a lineage trick).

    Faithful by construction: a deterministic decode of bytes the base already
    holds — no inference, no fetch, **no new version** (I1 is about bytes;
    these are unchanged; content_hash stays put). The aid is non-authoritative
    (ADR-0010 rule 3), and the current version's ledger entry gains
    `text_aid`/`extracted_by` — fields the format already allows, so no format
    change. What it never does: overwrite an existing aid, or re-stamp
    `derivation` on docs authored model-read for want of an aid — a stamp is
    the adapter's honest record of how IT read; `regenerate` is the consented
    path to change one.

    With `id`, backfills that one source (raising if it doesn't exist); without,
    sweeps every source and reports per-source outcomes.
    """
    root = Path(root)
    when = when or util._now()
    sources = root / "sources"
    dirs = sorted(d for d in sources.iterdir() if d.is_dir()) if sources.is_dir() else []
    results = []
    for sdir in dirs:
        meta_p = sdir / "meta.yml"
        if not meta_p.exists():
            continue
        meta = _load_yaml(meta_p)
        sid = meta.get("id", sdir.name)
        if id is not None and sid != id:
            continue
        results.append({"id": sid, **_reextract_one(sdir, meta, meta_p)})
        if results[-1]["action"] == "aided":
            _append_log(root, when,
                        f"reextract | aided | {sid} ({results[-1]['extracted_by']})")
    if id is not None and not results:
        raise ValueError(f"no source with id {id!r}")
    return {"checked": len(results),
            "aided": sum(1 for r in results if r["action"] == "aided"),
            "results": results}


def _reextract_one(sdir, meta, meta_p):
    canonical = current_canonical(sdir, meta)
    if canonical is None:
        return {"action": "skipped", "reason": "no held bytes (reference tier)"}
    if canonical.suffix.lower() in TEXT_SUFFIXES:
        return {"action": "skipped", "reason": "text-native; the canonical is its own text"}
    cur_n = int(meta.get("version", 1))
    entry = next((e for e in meta.get("history", [])
                  if isinstance(e, dict) and e.get("version") == cur_n), None)
    # Skip only when the aid exists AND the ledger covers it. An aid on disk
    # with no ledger entry is a crashed prior run (L13 flags it); falling
    # through re-extracts the identical text and commits the ledger — the
    # re-run IS the repair.
    if (sdir / "source-text.md").exists() and entry is not None and entry.get("text_aid"):
        return {"action": "skipped", "reason": "aid already present"}
    ex = extractors.for_format(canonical.suffix.lower())
    if ex is None:
        return {"action": "skipped", "reason": f"no extractor for {canonical.suffix}"}
    try:
        text = ex.extract(canonical.read_bytes())
    except Exception as e:  # bytes-only stays valid (ADR-0010 rule 5)
        return {"action": "skipped",
                "reason": f"extraction failed: {ex.name}: {type(e).__name__}: {e}"}
    if entry is not None:
        entry["text_aid"] = "source-text.md"
        entry["extracted_by"] = ex.name
    # T-115 ordering: aid lands first, meta commits last — a crash between the
    # two leaves an uncovered aid file, which L13 surfaces and a re-run repairs.
    tmp_aid = sdir / ".source-text.md.tmp"
    tmp_aid.write_text(text, encoding="utf-8")
    atomic_replace(tmp_aid, sdir / "source-text.md")
    tmp_meta = sdir / ".meta.yml.tmp"
    tmp_meta.write_text(_dump_yaml(meta), encoding="utf-8")
    atomic_replace(tmp_meta, meta_p)
    return {"action": "aided", "extracted_by": ex.name}
