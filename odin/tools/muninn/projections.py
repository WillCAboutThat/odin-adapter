...
import repo_constitution  # noqa: E402  (faithful manifest read-back for repo-coverage, T-196)
...
from .util import _append_log, _dump_yaml, _locked, _valid_id, atomic_replace  # noqa: E402
...
def regenerate_index(root):
    ...
    # index.jsonl — the machine-floor projection (ADR-0046, T-204): the same
    # frontmatter, one JSON record per doc, for no-AI tooling. A projection
    # artifact like llms.txt: computed, not format-binding, never load-bearing.
    (root / "index.jsonl").write_text(
        muninn_lint.render_index_jsonl(root, linter.docs), encoding="utf-8")
    return index
...
# --------------------------------------------------------------------------- #
# Reserved read ids (2026-08-19 adapter feedback): the control files a host
# without filesystem access needs, kept inside the op surface (T-159 spirit).
_RESERVED_READS = {
    "@instructions": "MUNINN.md",
    "@manifest": "muninn.yml",
    "@index": "index.md",
}


def _doc_text(root, id):
    """Resolve an id (doc or reserved control-file) to (meta dict, full text)."""
    root = Path(root)
    if id in _RESERVED_READS:
        p = root / _RESERVED_READS[id]
        if not p.exists():
            raise ValueError(f"{_RESERVED_READS[id]} does not exist in this base")
        return ({"id": id, "kind": "control", "type": "control",
                 "text_form": "file"},
    ...
    if d is None:
        raise ValueError(f"no doc with id {id!r} in this base "
                         f"(reserved ids: {', '.join(sorted(_RESERVED_READS))})")
    if d.kind == "source":
...
def _section_slice(text, section):
    """The character range of a markdown section: from the heading line whose
    text contains `section` (case-insensitive) to the next heading of the same
    or higher level. Deterministic; no heading match raises."""
    ...
    for line in lines:
        ...
        if m:
            if start is not None and len(m.group(1)) <= level:
                return start, pos
            if start is None and want in m.group(2).strip().lower():
                start, level = pos, len(m.group(1))
        pos += len(line)
    if start is None:
        raise ValueError(f"no heading matching {section!r} in {len(lines)} lines")
    return start, len(text)


def read_doc(root, id, offset=0, limit=20000, find_text=None, section=None):
    ...
    `truncated` says whether more remains past this page. `chars` is always the
    FULL text length — probe with limit=1 to size a doc before reading.

    Two deterministic jump aids (2026-08-19 adapter feedback — literal routing,
    never inference; strong claims still read the returned ranges):
    - `find_text`: case-insensitive literal search; returns `matches`
      [{offset, excerpt}] (cap 20) instead of a content page.
    - `section`: return the markdown section whose heading contains this text
      (to the next same-or-higher heading); `offset` in the result is the
      section's true offset, so paging and citation ranges compose.

    Reserved ids `@instructions` (MUNINN.md), `@manifest` (muninn.yml), and
    `@index` (index.md) read the control files (T-159: hosts with only the op
    surface). Read-only.

    Returns {id, kind, type, text_form, chars, offset, content, truncated}
    (or `matches`/`match_count` in find_text mode).
    """
    offset = max(0, int(offset))
    limit = int(limit)
    if limit <= 0:
        raise ValueError("limit must be positive")
    if find_text and section:
        raise ValueError("find_text and section are mutually exclusive")
    meta, text = _doc_text(root, id)
    head = {"id": meta["id"], "kind": meta["kind"], "type": meta["type"],
            "text_form": meta["text_form"], "chars": len(text)}

    if find_text:
        needle = find_text.lower()
        hay = text.lower()
        matches, pos = [], hay.find(needle)
        while pos != -1 and len(matches) < 20:
            lo = max(0, pos - 150)
            hi = min(len(text), pos + len(needle) + 150)
            matches.append({"offset": pos, "excerpt": text[lo:hi]})
            pos = hay.find(needle, pos + 1)
        more = pos != -1
        return {**head, "find_text": find_text, "matches": matches,
                "match_count": len(matches), "truncated_matches": more}

    if section:
        s, e = _section_slice(text, section)
        page = text[s + offset:min(e, s + offset + limit)]
        return {**head, "section": section, "offset": s + offset,
                "content": page,
                "truncated": s + offset + len(page) < e}

    page = text[offset:offset + limit]
    return {**head, "offset": offset, "content": page,
            "truncated": offset + len(page) < len(text)}


def read_many(root, ids, max_total_chars=100000):
    """Batched evidence reading (2026-08-19 adapter feedback): the mechanical
    round-trip saver, deliberately NOT a relevance-ranking packet assembler —
    scoping and judgment stay with the caller (ADR-0014's line). Reads each id
    in the caller's order until the shared character budget is spent; per-doc
    results carry the same fields as `read` plus `status` and, for derived
    docs, `sources` (provenance ids). Ids the budget never reached come back
    in `not_read` with their sizes, so the caller pages deliberately.
    Read-only."""
    if not ids:
    ...
    for id in ids:
        meta, text = _doc_text(root, id)
        d = meta.get("doc")
        row = {"id": meta["id"], "kind": meta["kind"], "type": meta["type"],
               "text_form": meta["text_form"], "chars": len(text)}
        if d is not None:
            row["status"] = d.data.get("status", "current")
            prov = d.data.get("sources") or []  # provenance frontmatter (I3)
            if prov:
                row["sources"] = [p.get("id") for p in prov if isinstance(p, dict)]
        if budget <= 0:
            not_read.append({"id": row["id"], "chars": row["chars"]})
            continue
        page = text[:budget]
        budget -= len(page)
        docs.append({**row, "offset": 0, "content": page,
                     "truncated": len(page) < len(text)})
    return {"docs": docs, "not_read": not_read,
            "budget_exhausted": bool(not_read) or budget <= 0}
...
# --------------------------------------------------------------------------- #
def find(root, query, type=None, include_superseded=False, project=None):
    ...
    capture's lineage rung and dedup-check already match on (T-045).

    Ordering is by **how much of the query the doc's name carries** (T-235
    follow-on): terms found in the identity fields — id, title, tags, origin
    locators (a `.sql` filename IS a name), and for sources the HELD format
    (the canonical suffix, non-markdown; what the base holds, not what the
    locator claims) — outrank terms found only in the abstract or body, then
    sources before derived before projects/decisions, then id. Being named by
    the query beats mentioning it; still deterministic field weighting, no
    inference (ADR-0012's line holds).
    ...
    **Superseded docs are skipped by default** (ADR-0041): retrieval serves
    ...
    for d in linter.docs:
        ...
        named = [d.id]
        if d.data.get("title"):
            named.append(str(d.data["title"]))
        named += [str(t) for t in (d.data.get("tags") or [])]
        if d.kind == "source":
            ...
            origin = d.data.get("origin") or {}
            named += [str(origin[k]) for k in ("ref", "upstream_ref")
                      if origin.get(k)]
            # The HELD format is identity too — same rule as the viewer's
            # file-type chip: what the base holds, not what the locator
            # claims; markdown (the base's own format) stays unmarked. A page
            # captured from an extensionless URL is findable as "html", and a
            # held .xlsx outranks a page that merely talks about xlsx files.
            canonical = current_canonical(d.path, d.data)
        ...
        if all(t in hay for t in terms):
            n_named = sum(1 for t in terms if t in named_hay)
            results.append({"id": d.id, "kind": d.kind, "type": d.type,
                            "title": d.data.get("title", d.id), "path": str(d.path),
                            # every term in the name = the doc IS what the query
                            # names; consumers (retrieve's union) rank on this
                            "matched": "name" if terms and n_named == len(terms)
                            else "mention",
                            "_named": n_named})
    results.sort(key=lambda r: (-r["_named"], order.get(r["kind"], 9), r["id"]))
    for r in results:
        del r["_named"]
    if project is not None:
        # Scoped retrieval (2026-08-19 adapter feedback): restrict to the
        # resolved working set — the same deterministic set math synthesize
        # uses (resolve_scope; unknown project raises there, never a silent
        # whole-base fallback). Projects/decisions aren't scope members, so a
        # scoped find is sources + derived only, by construction.
        members = set(resolve_scope(root, project)["members"])
        results = [r for r in results if r["id"] in members]
    return results
...
def write_project(root, id, *, title=None, add_members=None, remove_members=None,
    ...
    atomic_replace(tmp, ppath)  # atomic replace into place
...
def _split_repo_locator(ref):
    """A repo-scoped locator → (repo, region). The T-196 convention: a constitution
    carries the BARE `<repo-locator>`; an evidence capture qualifies it with
    `#<path>` (a whole file) or `#<path>#<slug>` (an anchored excerpt). Split on the
    first `#`: everything before is the repo; the remainder (or None) is the region.
    `region` keeps any `#<slug>`; the file `path` is `region` up to the next `#`."""
    if not ref:
        return None, None
    repo, sep, region = ref.partition("#")
    return repo, (region if sep else None)


def _norm_repo_path(path: str) -> str:
    """A repo-relative path reduced to a comparable key (T-228).

    Separators and casing are how the same file gets written two different ways:
    a manifest built on Windows records `stack\compose.yml` while the capture
    convention writes `stack/compose.yml`, and a literal glob segment records the
    pattern's casing rather than the file's. Both differences are about the host
    that read the repo, not about the repo, so neither should decide whether a
    file counts as covered.
    """
    return (path or "").replace("\\", "/").strip("/").lower()


def repo_coverage(root, repo=None):
    """Project the **honest footprint** of what this base holds about a repository — the
    repo analog of the `connectors` roster (T-196). A deterministic, faithful read of stored
    provenance and manifest text: it never walks the repo, never infers what *should* be
    covered, and never claims to understand the uncovered repo. It shows only what is *held*.

    Repo-sourced material is every source with `origin.system == "repo"` (ADR-0028 +
    the T-196 capture convention). Its `origin.ref` groups it: the **constitution** carries
    the bare `<repo-locator>`; **evidence** captures qualify it `#<path>[#<slug>]`. Per repo:

      - `constitution` — the mental-model source (id, content_hash) + the surfaces it holds,
        read back faithfully from the stored manifest header (`parse_manifest_surfaces`);
      - `covered` — the dedicated file/excerpt evidence captures (id, region, path, tier,
        upstream_ref), the current, quotable sources a code claim can ground in;
      - `concepts` — the derived docs grounded in ANY of this repo's sources (id, type,
        title) — the knowledge already synthesized, not the raw captures;
      - `references_not_captured` — constitution-named surface paths with NO dedicated
        evidence capture: the mental model LOCATES these files but the base holds no current
        source of its own for them. The T-142 "a miss is not absence" frontier made a value:
        it is exactly the **partial** class (the subsystem is placed; fetch the live file to
        ground), never silently read as full coverage.

    `repo` (a locator) scopes to one repo and ALWAYS returns exactly one entry for it —
    all-empty when the base knows nothing of it (the honest **unknown** answer, not an error;
    gap detection is a thin adapter classification on top). Omit `repo` for the roster: one
    entry per repo the base knows. Read-only; a pure projection of frontmatter + stored text,
    so it goes stale like `index.md`, never a durable registry.

    Returns `{"repo_filter": repo|None, "repos": [{repo, constitution, covered, concepts,
    references_not_captured}]}`, repos sorted by locator.
    """
    from collections import defaultdict
    root = Path(root)
    linter = Linter(root)
    ...
    # repo locator -> {"constitution": doc|None, "evidence": [(doc, region)]}
    groups: dict = defaultdict(lambda: {"constitution": None, "evidence": []})
    src_repo: dict = {}     # source id -> its repo locator (for the derived-doc join)
    for d in linter.docs:
        if d.kind != "source":
            continue
        origin = d.data.get("origin") or {}
        if origin.get("system") != "repo":
            continue
        locator, region = _split_repo_locator(origin.get("ref"))
        if locator is None:
            continue
        src_repo[d.id] = locator
        if region is None:
            groups[locator]["constitution"] = d       # bare locator = the constitution
        else:
            groups[locator]["evidence"].append((d, region))

    if repo is not None:
        groups.setdefault(repo, {"constitution": None, "evidence": []})  # unknown -> empty entry
        locators = [repo]
    else:
        locators = sorted(groups)

    # derived docs grouped by the repo their grounding sources belong to
    derived_by_repo: dict = defaultdict(list)
    for d in linter.docs:
        if d.kind != "derived":
            continue
        seen = set()
        for s in (d.data.get("sources") or []):
            sid = s.get("id") if isinstance(s, dict) else s
            loc = src_repo.get(sid)
            if loc and loc not in seen:
                seen.add(loc)
                derived_by_repo[loc].append(d)
    ...
    for locator in locators:
        ...
        constitution = None
        if cdoc is not None:
            manifest = source_text(cdoc.path, cdoc.data)
            constitution = {"id": cdoc.id,
                            "content_hash": cdoc.data.get("content_hash"),
                            "surfaces": repo_constitution.parse_manifest_surfaces(manifest)}

        covered, covered_paths = [], set()
        for d, region in sorted(g["evidence"], key=lambda e: e[0].id):
            origin = d.data.get("origin") or {}
            path = region.partition("#")[0]                    # region up to any #slug
            covered_paths.add(_norm_repo_path(path))
            covered.append({"id": d.id, "region": region, "path": path,
                            "tier": d.data.get("capture", "full"),
                            "upstream_ref": origin.get("upstream_ref")})

        concepts = [{"id": d.id, "type": d.type,
                     "title": d.data.get("title", d.id)}
                    for d in sorted(derived_by_repo.get(locator, []), key=lambda x: x.id)]

        # constitution-named surface paths with no dedicated evidence capture (the partial
        # frontier). topology's sentinel path is a shape, not a file — never a "reference".
        # Compared on normalized paths (T-228). A manifest already captured on
        # Windows holds host separators in its bytes, and a captured source is
        # immutable (I1) — so fixing the writer cannot fix the manifests already
        # in a base. Normalizing here is what makes an existing base read
        # honestly, not a belt-and-braces extra.
        named = {}
        if constitution:
            for s in constitution["surfaces"]:
                if s["label"] == "topology":
                    continue
                for p in s["paths"]:
                    named.setdefault(_norm_repo_path(p), p)    # report as recorded
        references_not_captured = sorted(
            orig for key, orig in named.items() if key not in covered_paths)

        repos.append({"repo": locator, "constitution": constitution,
                      "covered": covered, "concepts": concepts,
                      "references_not_captured": references_not_captured})

    return {"repo_filter": repo, "repos": repos}
