...
from .queue import queue_list  # noqa: E402
...
def status(root, as_of=None, aging_window_days=_AS_OF_WINDOW_DAYS):
    ...
    return {
        ...
        # pending work-queue tasks (ADR-0048/T-209): work the owner queued for
        # an executor session — surfaced here so any session that starts with
        # a status read learns the queue is waiting without being told.
        "pending_tasks": queue_list(root)["pending_count"],
...
def record_decision(root, id, *, body, title=None, status=None, evidence=None,
    ...
    # ADR-0051: a decision is the operator's accountable voice - approval IS
    # authorship for this one type (the owner signs). Core-written, type-derived.
    fm["authored_by"] = "operator"
    if evidence_list:
    ...
    atomic_replace(tmp, dpath)  # atomic replace into place
