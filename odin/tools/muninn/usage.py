...
def usage_log(root, op, *, scope=None, bytes_in=None, bytes_out=0, tokens=None,
    ...
    if ids:
        ...
        # T-211: the scope ids themselves, not just the count — the strongest
        # per-doc attention signal there is (an AI verb grounded in these).
        extra["ids"] = ids
    if note:
        extra["note"] = str(note)
...
# --------------------------------------------------------------------------- #
# Attention report (T-211) — which docs actually get touched, and what the
# untouched tail costs. The observability that must precede any dormancy /
# working-set design (the T-191 lesson: measure before building): the ledger's
# per-op records now carry the doc ids each read-path op touched, and this
# report joins them against the live base. Disposable-tier facts only — the
# ledger is per-clone and git-ignored, so this describes THIS machine's
# attention, honestly disclosed as such.
# --------------------------------------------------------------------------- #
#: ops whose ids mean a doc was OPENED or GROUNDED IN (deliberate attention)
_STRONG_ATTENTION_OPS = frozenset(
    {"read", "derive", "ask", "review", "synthesize", "challenge", "map"})
#: ops whose ids mean a doc was merely SURFACED in a result list
_WEAK_ATTENTION_OPS = frozenset({"find", "search", "retrieve", "query"})


def _parse_ts(ts):
    """An ISO `...Z` ledger timestamp (or bare date) to a date, else None."""
...
def attention_report(root, as_of=None, windows=(30, 60, 90)):
    """Join the ledger's per-doc touch records against the current base.

    Returns, per doc: last strong/weak touch and counts, plus its readable-byte
    footprint (the attention-tier cost of keeping it in the working set). Plus
    the aggregates that would justify (or refute) a dormancy design: how many
    docs — and how many bytes — went untouched inside each window.

    Honesty rails: the report states the ledger's own age (`ledger_age_days`)
    and marks any window the ledger hasn't lived through as `meaningful: false`
    — a two-week-old ledger makes every doc look untouched-for-90-days, and
    that absence must never masquerade as measurement (I5). Read-only,
    deterministic given (ledger bytes, base, as_of); time enters only via
    `as_of`, the `status` pattern."""
    root = Path(root)
    as_of_d = _parse_ts(as_of or util._now())
    if as_of_d is None:
        raise ValueError(f"as_of {as_of!r} is not an ISO date")
    ...
    for rec in read_usage_records(root):
        n_records += 1
        ts = rec.get("ts")
        if first_ts is None or (ts and str(ts) < str(first_ts)):
            first_ts = ts
        op = str(rec.get("op", "?"))
        strong = op in _STRONG_ATTENTION_OPS
        weak = op in _WEAK_ATTENTION_OPS
        if not (strong or weak):
            continue
        d = _parse_ts(ts)
        for i in (rec.get("ids") or []):
            t = touches.setdefault(str(i), {"strong_n": 0, "weak_n": 0,
                                            "last_strong": None, "last_weak": None})
            kind = "strong" if strong else "weak"
            t[f"{kind}_n"] += 1
            last = t[f"last_{kind}"]
            if d is not None and (last is None or str(d) > last):
                t[f"last_{kind}"] = str(d)
    ...
    for d in linter.docs:
        if d.kind == "manifest":
            continue
        if d.kind == "source":
            nbytes = _source_bytes(root, d.id)
        else:
            try:
                nbytes = d.path.stat().st_size
            except OSError:
                nbytes = 0
        total_bytes += nbytes
        t = touches.get(d.id, {})
        last = max((v for v in (t.get("last_strong"), t.get("last_weak")) if v),
                   default=None)
        docs.append({"id": d.id, "kind": d.kind, "type": d.type,
                     "title": d.data.get("title", d.id), "bytes": nbytes,
                     "strong_n": t.get("strong_n", 0), "weak_n": t.get("weak_n", 0),
                     "last_strong": t.get("last_strong"),
                     "last_weak": t.get("last_weak"), "last_touch": last})
    docs.sort(key=lambda r: (r["last_touch"] or "", r["id"]))

    ledger_start = _parse_ts(first_ts) if first_ts else None
    ledger_age = (as_of_d - ledger_start).days if ledger_start else 0

    def _untouched(days):
        cutoff = None
        from datetime import timedelta
        cutoff = str(as_of_d - timedelta(days=days))
        rows = [r for r in docs if r["last_touch"] is None or r["last_touch"] < cutoff]
        return {"days": days,
                "untouched_docs": len(rows),
                "untouched_bytes": sum(r["bytes"] for r in rows),
                "pct_docs": round(100 * len(rows) / len(docs), 1) if docs else 0.0,
                "pct_bytes": round(100 * sum(r["bytes"] for r in rows) / total_bytes, 1)
                             if total_bytes else 0.0,
                # a window the ledger hasn't lived through measures the ledger's
                # youth, not the base's dormancy — flagged, never hidden
                "meaningful": ledger_age >= days}

    never = [r for r in docs if r["last_touch"] is None]
    return {"as_of": str(as_of_d),
            "ledger_records": n_records,
            "ledger_age_days": ledger_age,
            "docs_total": len(docs),
            "docs_bytes": total_bytes,
            "never_touched": {"docs": len(never),
                              "bytes": sum(r["bytes"] for r in never)},
            "windows": [_untouched(w) for w in windows],
            "docs": docs}
