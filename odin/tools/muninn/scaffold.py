...
def init(root, name=None, when=None, allow_tool_root=False):
    ...
    **Soft-warn tool-repo guard (T-032/ADR-0032):** if the target sits inside ODIN's
    ...
    # The integrity knob is written present-and-ON for freshly scaffolded bases
    # (T-243, amending ADR-0029 in place per ADR-0025). ADR-0029 made L19 opt-in so
    # it "never forces hash churn on existing bases" — a rationale that cannot apply
    # to a base that does not exist yet: the Core stamps self_hash on every derived
    # write regardless, so enforcing from day one costs nothing and catches
    # out-of-band derived-doc edits at the first lint. The T-240 run is the fixture:
    # a body-poisoned derivative survived eight lint-clean sessions with this off.
    # Existing bases are never touched, and the knob remains a per-Muninn opt-out.
    manifest.write_text(
        f"muninn: {FORMAT_VERSION}\nname: {name}\ncreated_at: {when}\n"
        f"integrity:\n  derived_self_hash: true  # L19 on for new bases (T-243); set false to opt out\n",
        encoding="utf-8")
