...
# Every Core write commits by renaming a temp path into place — that rename IS
# the atomicity guarantee. On Windows it can fail for a reason POSIX has no
# equivalent of: any process holding a transient handle to the source or the
# destination (Defender scanning a file written milliseconds ago, the search
# indexer, a sync client) makes the rename raise ACCESS_DENIED or
# SHARING_VIOLATION, where POSIX permits it outright. It clears in milliseconds,
# so a bounded retry converts a spurious failure into a slight delay (T-232).
#
# This changes nothing about the operation: same rename, same atomicity, same
# single commit point. It only declines to give up on the first refusal.
_RENAME_RETRY_DELAYS = (0.01, 0.02, 0.05, 0.1, 0.2)   # ~0.38s before the last try
_TRANSIENT_WINERRORS = frozenset({5, 32})             # ACCESS_DENIED, SHARING_VIOLATION


def _rename_with_retry(op, src: Path, dst: Path) -> None:
    """Run `op` (Path.replace or Path.rename), retrying Windows' transient locks.

    Only a PermissionError carrying a transient Windows error code is retried.
    A POSIX PermissionError has no `winerror`, so it surfaces immediately and
    unchanged — a real permissions problem must never be spent 0.4s hiding.
    FileExistsError from `rename` is likewise never retried: it is a fact about
    the destination, not a lock, and it is load-bearing where we use it.
    """
    for delay in _RENAME_RETRY_DELAYS:
        try:
            ...
            if getattr(e, "winerror", None) not in _TRANSIENT_WINERRORS:
                raise
            time.sleep(delay)
    op(src, dst)      # last attempt: whatever it raises is the honest answer


def atomic_replace(src: Path, dst: Path) -> None:
    """`Path.replace` (atomic, overwrites) hardened against Windows lock flakes."""
    _rename_with_retry(Path.replace, Path(src), Path(dst))


def atomic_rename(src: Path, dst: Path) -> None:
    """`Path.rename` (atomic, REFUSES an existing dst) hardened the same way.

    Distinct from atomic_replace on purpose: capture's new-source move relies on
    failing when the destination already exists.
    """
    _rename_with_retry(Path.rename, Path(src), Path(dst))
