...
from .projections import connector_projection, drift_worklist, find, fingerprint, read_doc, regenerate_index, repo_coverage, reproject, resolve_scope, write_project  # noqa: E402
...
OPS = {
    ...
    "query": {
        ...
        "handler": lambda root, p: muninn_catalog.query(
    ...
    "graph": {
        "help": "project explicit relationships as neutral graph JSON "
                "(odin.graph.v1; T-208/ADR-0048)",
        "description": "Project the base's EXPLICIT relationship graph as "
                       "neutral, renderer-agnostic JSON (odin.graph.v1). "
                       "Every edge is a frontmatter fact — grounded_by "
                       "(provenance, with per-rung staleness), see_also, "
                       "member_of, evidence, superseded_by, excerpt_of "
                       "(ADR-0039 anchors), from_repo (T-196 locators) — "
                       "never an inference (no title-mention or similarity "
                       "edges; the known-vs-inferred line stays sharp, "
                       "ADR-0015). Read-only; `center`+`depth` restrict to "
                       "one doc's neighborhood, `kinds`/`relations` filter. "
                       "`cache_key` is a stat-sweep change detector for "
                       "layout caches, not a content hash.",
        "params": {
            "root": _ROOT_P,
            "center": {"type": "string",
                       "description": "Restrict to this doc's undirected "
                                      "neighborhood (unknown id errors)."},
            "depth": {"type": "integer",
                      "description": "Hops from center (default 1; ignored "
                                     "without center)."},
        ...
        "handler": lambda root, p: project_graph(
            root, center=p.get("center"), depth=p.get("depth"),
            kinds=p.get("kinds"), relations=p.get("relations")),
    },
    "project": {
