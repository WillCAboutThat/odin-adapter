...
from . import queue as queue_module  # noqa: E402
from .queue import queue_create, queue_list, queue_restatus  # noqa: E402
from .capture import anchor, anchor_check, capture, capture_file, capture_repo, dedup_check, log_drift_check, reextract, retier, source_status  # noqa: E402
from .decisions import lint_report, record_decision, status  # noqa: E402
from .graph import project_graph  # noqa: E402
from .derive import REVIEW_OUTCOMES, log_challenge, log_map, log_review, relink, stamp_derived, supersede, write_derived  # noqa: E402
from .projections import connector_projection, drift_worklist, find, fingerprint, read_doc, read_many, regenerate_index, repo_coverage, reproject, resolve_scope, write_project  # noqa: E402
from .scaffold import init  # noqa: E402
from .usage import _source_bytes, attention_report, log_usage, usage_html, usage_log, usage_report  # noqa: E402

import muninn_catalog  # noqa: E402  (catalog accelerator, ADR-0047 — tools/ root, like muninn_lint)
...
def _op_capture(root, p):
    ...
    if p.get("source_file"):
        ...
        return capture_file(root, p["id"], src.read_bytes(),
                            ...
                            allow_large=bool(p.get("allow_large")))
...
def _usage_derive(root, p, res, duration_ms=None):
    log_usage(root, "derive",
              ...
              # T-211: the sources this derivation READ — per-doc attention facts
              ids=list(p.get("sources") or []),
              **({"duration_ms": duration_ms} if duration_ms is not None else {}))


# --- attention facts (T-211): read-path hooks record WHICH docs were touched.
# Same best-effort disposable ledger as every usage record; the attention
# report joins these ids against the base to show what the untouched tail
# costs — the measurement that must precede any dormancy design.
def _usage_read(root, p, res, duration_ms=None):
    log_usage(root, "read", bytes_in=len(res.get("content") or ""),
              ids=[res.get("id") or p.get("id")],
              **({"duration_ms": duration_ms} if duration_ms is not None else {}))


def _usage_hits(op_name, key):
    """A usage hook for a retrieval op whose result carries a hit list under
    `key` — records the surfaced ids (weak attention: shown, not read)."""
    def hook(root, p, res, duration_ms=None):
        hits = (res.get(key) or []) if isinstance(res, dict) else []
        log_usage(root, op_name, hits=len(hits),
...
def _show_status(rep):
    print(f"freshness: {rep['freshness']}  ·  {rep['pending_candidates']} candidate(s) "
          f"·  {rep.get('pending_tasks', 0)} queued task(s) "
...
def _show_repo_coverage(res):
    """The footprint 'screen': per repo, the constitution + surfaces held, the evidence
    captures, the synthesized concepts, and the named-but-unevidenced frontier (T-196)."""
    if not res["repos"]:
        print("(no repo knowledge held" +
              (f" for {res['repo_filter']}" if res.get("repo_filter") else "") + ")")
        return
    for r in res["repos"]:
        con = r["constitution"]
        head = f"{r['repo']}  —  "
        head += (f"constitution {con['id']} ({len(con['surfaces'])} surface(s))"
                 if con else "NO constitution held")
        print(head)
        for c in r["covered"]:
            anchored = " (excerpt)" if "#" in c["region"] else ""
            print(f"    covered: {c['path']}{anchored}  [{c['id']}, tier={c['tier']}]")
        for k in r["concepts"]:
            print(f"    concept: {k['type']:8} {k['id']}  —  {k['title']}")
        for path in r["references_not_captured"]:
            print(f"    named, not evidenced: {path}")
    print(f"({len(res['repos'])} repo(s) known)")


def _show_attention(rep):
    print(f"attention (this clone's ledger, {rep['ledger_age_days']}d old, "
          f"{rep['ledger_records']} records) — {rep['docs_total']} docs, "
          f"{rep['docs_bytes']:,} readable bytes")
    nt = rep["never_touched"]
    print(f"never touched: {nt['docs']} doc(s), {nt['bytes']:,} bytes")
    for w in rep["windows"]:
        tag = "" if w["meaningful"] else "  [window exceeds ledger age — not yet meaningful]"
        print(f"untouched {w['days']:>3}d: {w['untouched_docs']:>4} doc(s)  "
              f"{w['untouched_bytes']:>10,} bytes  ({w['pct_docs']}% of docs, "
              f"{w['pct_bytes']}% of bytes){tag}")
...
# ---------------------------------------------------------------------------
# Mechanical read-only mode (2026-08-19 adapter feedback). ODIN_READ_ONLY=1 in
# the environment makes a session EFFECT-FREE, enforced at the shared
# dispatch chokepoints (run_op here; odin_mcp.dispatch on the MCP surface):
# every verb in MUTATING_VERBS is refused, and the disposable usage-ledger
# write is suppressed — so diagnostics (lint/find/read/status) run with zero
# filesystem effects, mechanically. `caller_can_write` describes permissions;
# this describes effects — the distinction the first external audit tripped
# over when lint's ledger record read as "the audit wrote something".
# `retrieve` additionally skips its best-effort semantic self-heal under the
# flag (muninn_semantic.retrieve) and says so in its warning.
# ---------------------------------------------------------------------------
MUTATING_VERBS = frozenset({
    "anchor", "capture", "capture-repo", "challenge-log", "decline-candidate",
    "derive", "drift-log", "index", "init", "map-log", "project",
    "promote-candidate", "queue-create", "queue-restatus", "record-decision",
    "reextract", "relink", "reproject", "retier", "review-log",
    "stage-candidate", "stamp", "supersede", "usage-log",
    # semantic tier (muninn_semantic.OPS) — write the disposable .odin index
    "refresh", "reindex",
})


def _scoped_query(root, p):
    """Catalog query, optionally restricted to a resolved project scope. The
    filter is a post-pass over the catalog's hits (the catalog stays
    scope-ignorant); `scoped_out` reports how many hits the scope removed, so
    a narrowed result is never mistaken for a small base."""
    res = muninn_catalog.query(
        root, fts=p.get("fts"), type=p.get("type"), kind=p.get("kind"),
        cites=p.get("cites"), origin_system=p.get("origin_system"),
        status=p.get("status"), limit=p.get("limit") or 50)
    project = p.get("project")
    if project:
        members = set(resolve_scope(root, project)["members"])
        hits = res.get("hits") or []
        kept = [h for h in hits if h.get("id") in members]
        res = {**res, "hits": kept, "scope": project,
               "scoped_out": len(hits) - len(kept)}
    return res


def read_only_active():
    return os.environ.get("ODIN_READ_ONLY", "").lower() in ("1", "true", "yes")


def refuse_if_read_only(verb):
    """Raise when a mutating verb is invoked in a declared read-only session."""
    if read_only_active() and verb in MUTATING_VERBS:
        raise ValueError(
            f"read-only session (ODIN_READ_ONLY=1): {verb!r} mutates the base "
            f"and was refused. Unset ODIN_READ_ONLY to write.")
...
OPS = {
    ...
    "capture": {
        ...
        "params": {
            ...
            "allow_large": {"type": "boolean",
                            "description": "Deliberately hold >=25 MB of source bytes in a "
                                           "full-tier capture (T-233). Without it, oversized "
                                           "captures are refused with the reference-tier "
                                           "alternative named — immutable bytes live in git "
                                           "history forever."},
    ...
    "reextract": {
        "help": "backfill text aids from HELD bytes for sources whose format "
                "has an extractor now (T-226; no new version, no fetch)",
        "description": "Backfill `source-text.md` aids from bytes the base "
                       "already holds, for sources captured bytes-only before "
                       "their format's extractor existed (T-226). Deterministic "
                       "decode — no inference, no fetch, NO new version (bytes "
                       "and content_hash unchanged); the current ledger entry "
                       "gains text_aid/extracted_by. Never overwrites an "
                       "existing aid and never re-stamps derivations "
                       "(`regenerate` is the consented path for that). With "
                       "`id` targets one source; without, sweeps the base and "
                       "reports per-source outcomes.",
        "params": {
            "root": _ROOT_P,
            "id": {"type": "string", "cli": {"positional": True},
                   "description": "One source to backfill (raises if absent); "
                                  "omit to sweep every source."},
        },
        "handler": lambda root, p: reextract(root, p.get("id")),
    },
    "anchor-check": {
    ...
    "queue-list": {
        "help": "enumerate the work queue (pre-cognition staging, ADR-0048) — "
                "pending first; optional status filter",
        "description": "Enumerate the work queue (`queue/`, ADR-0048): tasks "
                       "awaiting an executor session, pending first. A task is "
                       "scoped consent — it authorizes exactly the named "
                       "operations on the named inputs; results land as "
                       "candidates, never direct base writes. Read-only.",
        "params": {
            "root": _ROOT_P,
            "status": {"type": "string",
                       "enum": ["pending", "blocked", "done", "declined",
                                "cancelled"],
                       "description": "Filter to one lifecycle state."},
        },
        "handler": lambda root, p: queue_list(root, status=p.get("status")),
    },
    "queue-create": {
        "help": "author a task into queue/ — authoring is the scoped consent "
                "act (ADR-0048)",
        "description": "Author a work-queue task (`queue/`, ADR-0048). "
                       "Authoring IS the scoped consent act: the task "
                       "authorizes exactly the named operations on the named "
                       "inputs; executor results land on the candidates rail, "
                       "never as direct base writes. Never overwrites.",
        "params": {
            "root": _ROOT_P,
            "type": {"type": "string", "required": True,
                     "enum": list(queue_module.TASK_TYPES),
                     "cli": {"positional": True},
                     "description": "What kind of work this is."},
            "outcome": {"type": "string", "required": True,
                        "description": "The requested outcome, in the owner's "
                                       "words (prose or an op list)."},
            "inputs": {"type": "array", "items": {"type": "string"},
                       "cli": {"append": True, "flag": "--input"},
                       "description": "Immutable input refs: inbox items, "
                                      "source ids, a URL, a question."},
            "created_by": {"type": "string",
                           "description": "Authoring surface (web, an adapter "
                                          "session…; default core)."},
        },
        "handler": lambda root, p: queue_create(
            root, p["type"], p["outcome"], inputs=p.get("inputs"),
            created_by=p.get("created_by"), when=util._now()),
    },
    "queue-restatus": {
        "help": "move a queue task through its lifecycle — re-statused, never "
                "deleted; terminal states stay terminal",
        "description": "Move a work-queue task through its lifecycle "
                       "(pending → done | declined | blocked | cancelled; "
                       "blocked may resume). Rewrites the status line and "
                       "appends one log line to the task's own ## Log — tasks "
                       "are re-statused, NEVER deleted, and illegal "
                       "transitions are refused. Executors set done/declined/"
                       "blocked when they finish, decline, or park a task.",
        "params": {
            "root": _ROOT_P,
            "id": {"type": "string", "required": True,
                   "cli": {"positional": True},
                   "description": "The task-… id to re-status."},
            ...
            "note": {"type": "string",
                     "description": "Appended to the log line (why, or a "
                                    "pointer to the results — candidate ids)."},
            "actor": {"type": "string",
                      "description": "Who is re-statusing (web, an executor "
                                     "session…; default core)."},
        },
        "handler": lambda root, p: queue_restatus(
            root, p["id"], p["status"], note=p.get("note"),
            actor=p.get("actor"), when=util._now()),
    },
    "status": {
        "help": "on-load status surface: freshness · stale · pending candidates "
                "· pending queue tasks · captures-since-lint · aged "
                "time-relative facts (ADR-0034)",
        "description": "On-load status surface (ADR-0034): freshness (fingerprint "
                       "vs last lint), stale docs, pending candidates, pending "
                       "work-queue tasks (ADR-0048), captures-since-lint, and "
                       "aged time-relative (`as_of`) docs — read-only, one call "
                       "for a single consolidated nudge. Pass `as_of` (today) "
                       "to age as_of docs.",
        "params": {
            "root": _ROOT_P,
            "as_of": {"type": "string",
    ...
    "index": {
        "help": "rebuild index.md (+ index.jsonl, llms.txt) as pure projections "
                "of frontmatter (SPEC §5.3, ADR-0046)",
        "description": "Rebuild the projection artifacts from document "
                       "frontmatter (deterministic, idempotent, no prose "
                       "authored): index.md for the human skim, index.jsonl "
                       "for no-AI tooling (ADR-0046), llms.txt for generic "
                       "agents (T-168).",
        "params": {"root": _ROOT_P},
    ...
    "repo-coverage": {
        "help": "project the honest footprint of what the base holds about a repo "
                "— constitution, evidence captures, concepts, and the "
                "named-but-unevidenced frontier (the repo analog of connectors; T-196)",
        "description": "Project the honest FOOTPRINT of what this base holds about a "
                       "repository (T-196) — the repo analog of `connectors`. A "
                       "deterministic, faithful read of stored provenance and manifest "
                       "text: it never walks the repo and never infers what SHOULD be "
                       "covered. Repo-sourced material is every `origin.system: repo` "
                       "source (ADR-0028 + the T-196 convention); its locator groups it — "
                       "the constitution carries the bare `<repo>`, evidence captures "
                       "qualify it `#<path>[#<slug>]`. Per repo it reports: the "
                       "`constitution` (mental-model source + the surfaces it holds), "
                       "`covered` (dedicated file/excerpt evidence captures — the "
                       "current, quotable sources), `concepts` (derived docs grounded in "
                       "the repo's sources), and `references_not_captured` "
                       "(constitution-named surfaces with no dedicated capture — the "
                       "PARTIAL frontier a code question falls into: the subsystem is "
                       "located, fetch the live file to ground). With `repo` it scopes "
                       "to one locator and ALWAYS returns one entry — all-empty is the "
                       "honest UNKNOWN answer, never an error (gap detection is a thin "
                       "adapter classification on top). Read-only; a projection like "
                       "index.md — goes stale, never a durable registry.",
        "params": {"root": _ROOT_P,
                   "repo": {"type": "string",
                            "description": "A repo locator (the constitution's "
                                           "origin.ref) to scope to; omit for the "
                                           "roster of every repo the base knows.",
                            "cli": {"positional": True, "nargs": "?"}}},
        "handler": lambda root, p: repo_coverage(root, repo=p.get("repo")),
        "presenter": _show_repo_coverage,
    },
    "usage": {
    ...
    "attention": {
        "help": "which docs actually get touched, and what the untouched tail "
                "costs (T-211; per-clone ledger facts)",
        "description": "Join the disposable usage ledger's per-doc touch "
                       "records against the current base: per doc, the last "
                       "strong touch (read / grounded in) and weak touch "
                       "(surfaced in a result list) with counts and its "
                       "readable-byte footprint; plus the aggregate that "
                       "would justify or refute a dormancy design — how many "
                       "docs, and how many bytes, went untouched inside each "
                       "window (30/60/90d). Honest about its own limits: the "
                       "ledger is per-clone (this machine's attention only), "
                       "and any window the ledger hasn't lived through is "
                       "marked meaningful:false rather than reported as "
                       "dormancy. Read-only; time enters only via as_of.",
        "params": {
            "root": _ROOT_P,
            "as_of": {"type": "string",
                      "description": "Today (ISO date) — the windows' anchor; "
                                     "defaults to now."},
        },
        "handler": lambda root, p: attention_report(root, as_of=p.get("as_of")),
        "presenter": _show_attention,
    },
    "review-log": {
        "help": "record a completed review pass in the append-only log "
                "(history, never a stored verdict; T-216)",
        "description": "Record a completed `review` pass in the append-only "
                       "log: 'review | scope=<scope>: clean|findings-surfaced "
                       "[detail]'. The challenge-log precedent — history a "
                       "reader can consult (\"this scope was re-read "
                       "adversarially, on these dates\"), never a verdict the "
                       "format stores: no doc mark, no status field, no trust "
                       "score, because an AI blessing rots (ADR-0014). The "
                       "outcome is deliberately QUALITATIVE, never a finding "
                       "count — a deterministic-looking tally would launder "
                       "judgment as fact (ADR-0026), which is the overreach "
                       "review exists to hunt. `clean` records what the pass "
                       "reported, never a warranty. Run it as the close step "
                       "of a review, after any consented heals.",
        "params": {
            "root": _ROOT_P,
            "outcome": {"type": "string", "required": True,
                        "enum": list(REVIEW_OUTCOMES),
                        "cli": {"positional": True},
                        "description": "What the pass reported: clean (nothing "
                                       "a skeptical reader would question) or "
                                       "findings-surfaced."},
            "scope": {"type": "string",
                      "description": "What was reviewed (a project id, a doc "
                                     "id, or omit for the whole base)."},
            "detail": {"type": "string",
                       "description": "One line of context in the reviewer's "
                                      "own hedged words (what was re-read, "
                                      "what was offered)."},
        },
        "handler": lambda root, p: log_review(root, scope=p.get("scope"),
                                              outcome=p["outcome"],
                                              detail=p.get("detail")),
    },
    "challenge-log": {
    ...
    "find": {
        ...
        "description": "Deterministic retrieval: docs whose id/title/abstract/"
                       ...
                       "a valid query. Ordered by how much of the query the "
                       "doc's NAME carries (id/title/tags/locators, and a "
                       "source's HELD format: a .sql filename or a held .html "
                       "canonical is a name), then kind, then id: being named "
                       "by the query beats mentioning it. The AI-free floor "
                       "(ADR-0014) — no "
                       ...
                       "reporting absence (T-142). LITERAL MATCHING ONLY: when "
                       "the query is about MEANING rather than exact tokens, "
                       "reach for `retrieve` instead — it unions this floor "
                       "with semantic candidates and degrades back to it "
                       "mechanically, so it never recalls less (measured "
                       "2026-07-30: find 43% vs retrieve 100%@10 on "
                       "reader-vocabulary probes). This op stays the AI-free "
                       "durability floor (ADR-0014), not the default strategy.",
        "params": {
            "root": _ROOT_P,
            ...
            "project": {"type": "string",
                        "description": "Restrict hits to this project's resolved "
                                       "working set (members ∪ global views — the "
                                       "same set math as `resolve`). Unknown "
                                       "project errors; never a silent whole-base "
                                       "fallback."},
        },
        ...
        "handler": lambda root, p: (lambda hits: {"matches": hits,
            ...
            find(root, p["query"], type=p.get("type"),
                 ...
                 project=p.get("project"))),
        "usage": _usage_hits("find", "matches"),
        "presenter": _show_find,
    },
    "query": {
        "help": "structured + full-text catalog query (ADR-0047); degrades to find",
        "description": "Query the disposable catalog accelerator "
                       "(.odin/catalog.db, SQLite+FTS5): full-text `fts` terms "
                       "AND structured filters (type, kind, cites=<source-id>, "
                       "origin_system, status). Self-heals (refreshes changed "
                       "docs before ranking) and degrades MECHANICALLY to the "
                       "AI-free `find` walk on any catalog failure — the "
                       "result's `via` says which surface answered (T-090 "
                       "pattern). Proposes candidates to read, never grounds "
                       "(ADR-0027 §2); the file walk stays the arbiter. This "
                       "is the FACETED lane, and it is NOT part of "
                       "`retrieve`'s union: the structured filters "
                       "(cites/origin_system/status/kind) express the class of "
                       "question neither `find` nor `retrieve` can ask, so "
                       "reach here for 'which docs cite src-x' — not as a "
                       "synonym for search (measured 2026-07-30: structured "
                       "filters 7/7 exact, at ~34x less context than an index "
                       "read; its `fts` lane scored 55% vs semantic's 100% on "
                       "vocabulary gaps, so `fts` is not the synonym tool).",
        "params": {
            "root": _ROOT_P,
            "fts": {"type": "string",
                    "description": "Full-text terms (AND-joined, matched as "
                                   "literal phrases over title/abstract/tags/"
                                   "body)."},
            "type": {"type": "string",
                     "description": "Restrict to a derived-doc frontmatter "
                                    "type (summary, entity, insight, ...)."},
            "kind": {"type": "string",
                     "description": "Restrict to a doc kind: source | derived "
                                    "| project | decision."},
            "cites": {"type": "string",
                      "description": "Only derived docs whose provenance "
                                     "cites this source id."},
            "origin_system": {"type": "string",
                              "description": "Only sources captured from this "
                                             "origin system (url, repo, file, "
                                             "...)."},
            "status": {"type": "string",
                       "description": "Restrict to a frontmatter status "
                                      "(current, superseded, ...)."},
            "limit": {"type": "integer",
                      "description": "Max hits (default 50, cap 500)."},
            "project": {"type": "string",
                        "description": "Restrict hits to this project's resolved "
                                       "working set (post-filter over the same "
                                       "set math as `resolve`; unknown project "
                                       "errors)."},
        },
        "handler": lambda root, p: _scoped_query(root, p),
        "usage": _usage_hits("query", "hits"),
    },
    "graph": {
        "help": "project explicit relationships as neutral graph JSON — "
                "provenance/EVIDENCE disclosure and citation audit "
                "(odin.graph.v1; T-208/ADR-0048/ADR-0052)",
        "description": "Project the base's EXPLICIT relationship graph as "
                       "neutral, renderer-agnostic JSON (odin.graph.v1) — "
                       "THE op for EVIDENCE disclosure, PROVENANCE audit, "
                       "and CITATION tracing: for selected docs, what "
                       "sources they declare, whether that grounding is "
                       "current or stale, and which decisions connect. The "
                       "audit pipeline (ADR-0052): retrieve → you select "
                       "seeds → graph(centers=…, direction='outbound', "
                       "relations=['grounded_by','evidence','superseded_by']) "
                       "→ read-many → you read and judge. Every edge is a "
                       "frontmatter fact — grounded_by (provenance, with "
                       "per-rung staleness), see_also, member_of, evidence, "
                       "superseded_by, excerpt_of (ADR-0039), from_repo "
                       "(T-196) — never an inference (no title-mention or "
                       "similarity edges; ADR-0015). Direction follows the "
                       "stored edge; 'both' (default) is the undirected "
                       "neighborhood. Capped results say so honestly: "
                       "`complete` means complete for THESE seeds/relations/"
                       "direction/depth/caps — never complete evidence for a "
                       "question, never base validity (lint is the "
                       "authority; `unresolved_edges` is courtesy "
                       "diagnostics). `unexpanded_frontier` ids are "
                       "re-seedable — that IS the continuation; no cursor. "
                       "Read-only; `cache_key` is a stat-sweep change "
                       "detector for layout caches, not a content hash.",
        "params": {
            "root": _ROOT_P,
            "center": {"type": "string",
                       "description": "Restrict to this doc's neighborhood "
                                      "(unknown id errors; exclusive with "
                                      "centers)."},
            "centers": {"type": "array", "items": {"type": "string"},
                        "description": "Several seed docs — the deduped "
                                       "union of their neighborhoods "
                                       "(ADR-0052; you choose the seeds, "
                                       "the Core never does).",
                        "cli": {"flag": "--center-id", "append": True}},
            "depth": {"type": "integer",
                      "description": "Hops from the seeds (default 1; "
                                     "ignored without seeds)."},
            "direction": {"type": "string",
                          "enum": ["both", "outbound", "inbound"],
                          "description": "Traversal along the STORED edge: "
                                         "outbound = source-field→target "
                                         "(a derived doc's grounded_by "
                                         "reaches its sources and stops); "
                                         "inbound = the reverse; both "
                                         "(default) = undirected. "
                                         "Undirected relations (see_also) "
                                         "behave the same in every mode."},
            "max_nodes": {"type": "integer",
                          "description": "Cap on traversed nodes (depth "
                                         "bounds path length, not fan-out). "
                                         "Capped ⇒ complete:false + "
                                         "re-seedable frontier."},
            "max_edges": {"type": "integer",
                          "description": "Cap on returned edges (stable "
                                         "order decides survival, never "
                                         "relevance)."},
            "project": {"type": "string",
                        "description": "Annotate nodes with in_scope "
                                       "against this project's resolved "
                                       "working set — annotation, NEVER "
                                       "suppression: out-of-scope grounding "
                                       "sources stay disclosed."},
            "kinds": {"type": "array", "items": {"type": "string"},
                      "description": "Keep only these node kinds (source | "
                                     "derived | project | decision); seeds "
                                     "always survive.",
                      "cli": {"flag": "--kind", "append": True}},
            "relations": {"type": "array", "items": {"type": "string"},
                          "description": "Keep only these edge relations "
                                         "(grounded_by, see_also, member_of, "
                                         "evidence, superseded_by, excerpt_of, "
                                         "from_repo). Filters apply BEFORE "
                                         "the walk.",
                          "cli": {"flag": "--relation", "append": True}},
        },
        "handler": lambda root, p: project_graph(
            root, center=p.get("center"), centers=p.get("centers"),
            depth=p.get("depth"), kinds=p.get("kinds"),
            relations=p.get("relations"),
            direction=p.get("direction") or "both",
            max_nodes=p.get("max_nodes"), max_edges=p.get("max_edges"),
            project=p.get("project")),
    },
    "project": {
        ...
        "params": {
            ...
            "limit": {"type": "integer",
                      "description": "Max characters returned (default 20000); "
                                     ...
                                     "`chars` is always the FULL length — probe "
                                     "with limit=1 to size before reading."},
            "find_text": {"type": "string",
                          "description": "Case-insensitive LITERAL search inside "
                                         "the doc: returns matches [{offset, "
                                         "excerpt}] (cap 20) instead of a page — "
                                         "a deterministic jump aid; read the "
                                         "offsets to ground."},
            "section": {"type": "string",
                        "description": "Return the markdown section whose "
                                       "heading contains this text (to the next "
                                       "same-or-higher heading). Result offset "
                                       "is the section's true offset."},
        },
    ...
    "read-many": {
        "help": "batched evidence reading: several docs in one call under a "
                "shared character budget (round-trip saver)",
        "description": "Read several docs in ONE call under a shared character "
                       "budget — the mechanical round-trip saver for evidence "
                       "gathering (2026-08-19 adapter feedback). Reads ids in "
                       "the caller's order until max_total_chars is spent; each "
                       "doc carries the same fields as `read` plus `status` "
                       "and, for derived docs, `sources` (provenance ids). Ids "
                       "the budget never reached return in `not_read` with "
                       "their sizes for deliberate paging. Deliberately NOT a "
                       "relevance-ranking packet assembler: what to read and "
                       "what it means stay the caller's judgment (ADR-0014). "
                       "Reserved ids (@instructions/@manifest/@index) work "
                       "here too. Read-only.",
        "params": {
            "root": _ROOT_P,
            "ids": {"type": "array", "items": {"type": "string"},
                    "required": True,
                    "description": "Doc ids, read in this order.",
                    "cli": {"positional": True, "nargs": "+"}},
            "max_total_chars": {"type": "integer",
                                "description": "Shared budget across all docs "
                                               "(default 100000)."},
        },
        "handler": lambda root, p: read_many(
            root, p["ids"], max_total_chars=p.get("max_total_chars") or 100000),
    },
    "resolve": {
...
def mcp_tools(ops, prefix="odin_"):
    ...
    for verb, spec in ops.items():
        ...
        for name, ps in spec["params"].items():
            ...
            if ps.get("required"):
                req.append(name)
        # Mutability is capability metadata (2026-08-19 adapter feedback):
        # every tool self-declares whether it can change the base, so a
        # read-only session knows the contract before calling.
        effect = ("MUTATES the base — refused under ODIN_READ_ONLY=1."
                  if verb in MUTATING_VERBS else
                  "Read-only (no base mutation).")
        tools.append({"name": prefix + verb.replace("-", "_"),
                      "description": spec["description"] + " [" + effect + "]",
...
def run_op(ops, verb, root, params):
    ...
    spec = ops[verb]
    refuse_if_read_only(verb)
    t0 = time.perf_counter()
    res = spec["handler"](root, params)
    duration_ms = round((time.perf_counter() - t0) * 1000, 3)
    if read_only_active():
        return res  # effect-free: no ledger write either
    try:
        hook = spec.get("usage")
        if hook is not None:
