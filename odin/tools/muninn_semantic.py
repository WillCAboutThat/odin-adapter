...
def retrieve(root, query, *, k=10, model=None, url=DEFAULT_URL, embed=None,
    ...
    misses a synonym. Ranking is by signal agreement (T-235): hits both tiers
    surfaced first, then literal-only matches, then semantic-only proposals —
    deduped by id, each tier keeping its internal order.
    ...
    **Self-healing (ADR-0027, refined 2026-07-09):** before ranking, `retrieve` runs a
    ...
    # Scoped retrieval (2026-08-19 adapter feedback): restrict ranking to the
    # resolved working set (same set math as `resolve`; unknown project raises
    # there). In-scope hits rank normally; the top few OUT-of-scope hits are
    # kept under a separate, labeled key — cross-project serendipity stays
    # available, it just stops polluting the scoped ranking.
    scope_members = (set(core.resolve_scope(root, project)["members"])
                     if project is not None else None)

    def _scoped(rows):
        if scope_members is None:
            return rows, None
        return ([r for r in rows if r["id"] in scope_members],
                [r for r in rows if r["id"] not in scope_members][:5])

    def _result(via, backend, rows, warning, **extra):
        hits, outside = _scoped(rows)
        out = {"via": via, "backend": backend, "hits": hits,
               "warning": warning, **extra}
        if project is not None:
            out["scope"] = project
            out["outside_scope"] = outside
        return out

    def _find_rows():
        return [{"id": h["id"], "type": h["type"],
                 "title": h.get("title", h["id"]), "path": h["path"],
                 "matched": h.get("matched", "mention"),
                 "source": "find"} for h in find_hits]

    if core.read_only_active():
        # Effect-free session: the self-heal WRITES the disposable index, so
        # it is skipped — transparently (§I5), like every other degradation.
        warn = ("read-only session: semantic self-heal skipped; docs ingested "
                "since the last embed are find-reachable only")
    else:
        warn = refresh(root, model=model, url=url, embed=embed,
                       timeout=QUERY_TIMEOUT)["warning"]  # self-heal; never raises,
        # and on the INTERACTIVE budget (T-151): a cold backend costs seconds
        # here, never the bulk ceiling — docs left behind stay find-reachable
        # + warned.

    if index_info(root)["count"] == 0:          # still nothing embedded → floor only
        return _result("find", "no-index", _find_rows(), warn)
    try:
        ...
        return _result("find", "unavailable", _find_rows(), warn,
                       backend_detail=str(e))

    # Rank by SIGNAL STRENGTH (T-235): literal NAME matches, then both-tiers
    # agreement, then literal mentions, then semantic-only proposals. A doc the
    # query NAMES — id, title, tags, a held file's origin.ref — IS the thing
    # asked for; sources ground, summaries accelerate, so the held .sql file
    # outranks the summary titled after it (sources are never embedded, so the
    # union is the only place that ordering can be restored). A semantic top-k
    # always returns k neighbors whatever their quality, which is why the old
    # proposals-first order buried exact filename matches under noise.
    # Vocabulary-gap queries have an empty literal floor: pure-semantic order.
    find_ids = {h["id"] for h in find_hits}
    sem_by_id = {h["id"]: h for h in sem_hits}
    name_rows, both_rows, mention_rows, sem_rows = [], [], [], []
    seen = set()
    for h in _find_rows():                      # find's order: name-coverage,
        s = sem_by_id.get(h["id"])              # sources before derived
        if s is not None:
            h = {**h, "source": "semantic+find", "score": s["score"]}
        if h.get("matched") == "name":
            name_rows.append(h)
            seen.add(h["id"])
    for h in sem_hits:                          # semantic order for the rest
        if h["id"] in seen:
            continue
        row = {"id": h["id"], "type": h["type"], "title": h["title"],
               "path": h["path"], "score": h["score"]}
        row["source"] = "semantic+find" if h["id"] in find_ids else "semantic"
        (both_rows if h["id"] in find_ids else sem_rows).append(row)
        seen.add(h["id"])
    mention_rows = [h for h in _find_rows() if h["id"] not in seen]
    merged = name_rows + both_rows + mention_rows + sem_rows
    return _result("semantic+find", "up", merged, warn)
...
def _log_hit_usage(root, op, hits, duration_ms=None):
    """Attention facts (T-211): record the surfaced ids to the disposable usage
    ledger — weak attention (shown in a result list, not read). Same ledger,
    same best-effort contract as every usage record."""
    from muninn.usage import log_usage
    log_usage(root, op, hits=len(hits),
...
OPS = {
    ...
    "search": {
        "description": "Semantic retrieval: top-k derived docs by cosine "
                       ...
                       "is a literal token — and note most callers want "
                       "`odin_retrieve` instead of either: it unions this with "
                       "`find` so nothing is missed either way and it survives a "
                       "down backend. Reach for bare `search` only when you "
                       "specifically want semantic candidates ALONE (measured "
                       "2026-07-25: every gap probe chose retrieve, never bare "
                       "search).",
        "params": {
            "root": _ROOT_P,
        ...
        "handler": lambda root, p: search(
            ...
            url=p.get("url") or DEFAULT_URL),
        "usage": lambda root, p, res, duration_ms=None: _log_hit_usage(
            root, "search", res if isinstance(res, list) else [],
    ...
    "retrieve": {
        ...
        "params": {
            ...
            "project": {"type": "string",
                        "description": "Restrict ranked hits to this project's "
                                       "resolved working set (same set math as "
                                       "`resolve`; unknown project errors). The "
                                       "top few out-of-scope hits still return "
                                       "under the labeled `outside_scope` key."},
            "model": _MODEL_P,
            "url": _URL_P,
        },
        "handler": lambda root, p: retrieve(
            root, p["query"], k=p.get("k", 10), model=p.get("model"),
            url=p.get("url") or DEFAULT_URL, project=p.get("project")),
        "usage": lambda root, p, res, duration_ms=None: _log_hit_usage(
            root, "retrieve", (res or {}).get("hits") or [],
            duration_ms=duration_ms),
    },
    "refresh": {
