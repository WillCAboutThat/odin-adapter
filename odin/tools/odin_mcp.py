...
# ///
# ^ PEP-723 inline metadata (ADR-0031): `uv run --script` provisions Python + deps
...
#   pypdf + openpyxl ride along for the extractor registry (ADR-0010): the pdf and
#   xlsx-structured extractors exist in the Core, but under the plugin's isolated
#   uv env they were UNAVAILABLE at runtime, so PDFs/XLSX silently fell to
#   bytes-only + model-read. Found live by the T-240 smoke tier (finding F3, where
#   an arm agent 'fixed' it by editing the installed plugin cache - an edit the
#   next plugin update wipes). Deterministic extraction must not depend on what
...
# Both tables: the semantic tier's retrieval ops carry attention hooks too (T-211).
_USAGE_HOOKS = {_mcp_name(v): spec["usage"]
                for ops in (core.OPS, semantic.OPS)
                for v, spec in ops.items() if spec.get("usage")}
...
def dispatch(op, params):
    ...
    # Mechanical read-only mode (ODIN_READ_ONLY=1): same chokepoint rule as
    # the CLI's run_op — a mutating verb is refused, so a declared read-only
    # session is EFFECT-free, not just permission-limited.
    core.refuse_if_read_only(_VERB_BY_TOOL[op])
    params = params or {}
...
def _note_usage(op, params, result=None, duration_ms=None):
    ...
    if core.read_only_active():
        return  # effect-free session: the ledger write is an effect too
    try:
        ...
        if hook is not None:
            # pass the result as-is: search's hook takes its LIST result;
            # dict-indexing hooks only ever run on dict-returning ops
            hook(root, params, {} if result is None else result,
