...
**Anyone may implement this specification, in any language, for any purpose,
commercial or not, without permission, notice, or fee.** Conformance requires
nothing of the implementer beyond satisfying the rules stated here: no license
grant back, no attribution to claim conformance, and no obligation to use, ship,
or depend on Odin's own code.

That permission is the point rather than a courtesy. The format's central
promise is that a Muninn stays readable, verifiable, and useful **with no AI and
no vendor** (ADR-0008), and it is frozen so that promise survives (ADR-0037) —
none of which means anything if reading or writing the format required someone's
blessing. A base is plain Markdown, links, and hashes in a directory; the rules
below are what make it *checkable*, and they are yours to implement.

This document and the schemas that project it (`contracts/*.schema.json`) are
distributed under the repository's MIT license, so you may also copy, quote, and
adapt their text on those terms. The permission above is broader and independent:
it concerns building software that reads or writes the format, which needs no
license at all.
...
- **derived / project / decision / manifest** docs contribute a hash of their
  file bytes with **line endings normalized** (CRLF → LF, ADR-0049) — a
  document's line endings are not its content, so the same base fingerprints
  identically on every platform and a Muninn read from two machines is not
  permanently "drifted" on one of them;
...
authored_by: adapter               # optional (ADR-0051) - Core-written; derived prose is the adapter's accountable voice, by type (L26 keeps it coherent)
...
Beside the index the Core also emits **`llms.txt`** (T-168), a small routing file in the emerging convention generic AI agents fetch to orient in an unfamiliar repository: the base's name, a one-paragraph self-description, and links to `MUNINN.md` and `index.md`. It is a projection artifact like the index — computed, deterministic, no prose authored — and is **not load-bearing for conformance**: a 1.x base without it is fully conformant, and a reader that ignores it loses nothing.

The Core also emits **`index.jsonl`** (ADR-0046, T-204) — the **machine-floor** twin of the human-facing index: one compact JSON record per registered document (id, kind/type, title, abstract, tags, status, provenance source ids + hashes, origin facts, path), deterministically ordered (sources, derived, projects, decisions; id within each), keys sorted, every value a faithful copy of frontmatter. Its consumer is **no-AI tooling** — scripts, CI, migration tools — which should not have to parse Markdown or walk frontmatter to enumerate a base. Same posture as `llms.txt`: a projection artifact, not format-binding, not load-bearing for conformance. Where the file exists, the linter verifies it is a faithful render of frontmatter (L22) — each projection is checked against frontmatter, never against a sibling projection. The machine-readable record shape is described by `contracts/index-record.schema.json`.
...
the whole memory inspectable. **Only operations write it** — each line is
emitted by the op that performed it, through the Core's single append-mode
writer; no adapter, skill, or human authors an entry, and an event that
deserves the log reaches it through a recorder op (`drift-log`, `map-log`,
`challenge-log`, `record-decision`) rather than an editor. Do not confuse the
log with the index (§5.3): the index projects what the base *holds*, the log
records what *happened*, so a logged operation whose artifact was later removed
is not an inconsistency to repair in the log. Because the log is excluded from
the content fingerprint (§4.4), a hand-edit leaves no other trace — L23 checks
what it can (entry shape and verb vocabulary), and cannot see a deletion at all.
...
authored_by: operator                  # optional (ADR-0051) - Core-written; a decision is the operator's accountable voice (approval IS authorship for this one type)
...
| L23 log-entry-shape | **Warn only** (a pre-existing base may carry any log content, so none can fail): every `log.md` line that opens `## [` parses as an operation entry — an ISO timestamp, then a verb drawn from the vocabulary Core ops actually write. Catches the shapes a hand-edit leaves (an invented verb, a mangled stamp); a deletion is invisible by construction. The log is excluded from the fingerprint (§4.4), so this is its only integrity check | T-215, §5.4 |
| L24 path-as-body | **Warn only** (the L23 posture: a pre-existing base may carry such a body, so none can newly fail): a derived doc's body is not a single whitespace-free line shaped like a filesystem path (rooted, UNC, `~/`, or dot-relative). Catches a derive handed the *path* of the authored text instead of the text - stored faithfully, stamped, structurally valid, content garbage (the Core cannot know a path is not prose; the linter can flag the shape). Deliberately narrow, precision over recall: a path containing spaces escapes it | T-241, §5 |
| L25 mojibake-heuristic | Adapter-authored fields (a derived/decision doc's title, abstract, and body; a project's title) contain no double-encoding signature: character sequences produced when UTF-8 is decoded as cp1252 and re-encoded (`â€`…, `Ã©`, `Â³`, …). Advisory (**warn**): the linter names the likely cause (a host console/codepage chain) and the honest repair (re-derive, the regenerate discipline). Never checked in sources at all: bytes and text aids are faithful capture, and a source has no authored fields (the index borrows its description from the covering summary) | T-166 (as L21; renumbered by T-251 — the id collided with supersession-coherence), I5 |
| L26 authored-by-coherence | **Warn only** (optional field; absent docs are fine): a present `authored_by` matches the doc type's structural authority - `adapter` on derived docs, `operator` on decisions (ADR-0051: authorship is by type, the field is Core-written to make it glanceable; a divergent value is a hand-edit or a bug). Never checked on sources (faithful capture has no authored voice) | ADR-0051, I5 |
