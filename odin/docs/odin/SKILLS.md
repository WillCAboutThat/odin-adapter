...
3. **Choose the capture tier.** Prefer `full` — copy the bytes into
   ...
   **A capture sourced from a repo carries the repo pointer (T-196):** whole
   file or excerpt, stamp `origin.system: repo` and a repo-scoped locator
   (`<repo-locator>#<path>`, and `#<excerpt-slug>` for an excerpt) — the same
   `origin.system: repo` a `capture-repo` constitution uses (which keeps the
   bare `<repo-locator>`), so demand-driven code captures are groupable back to
   their repo, the footprint a `repo-coverage` read projects.
...
5. **Derive** (synthesis). Produce a summary grounded **only** in this source
   ...
   **for findability** (see the discipline below, ADR-0012). Set
   ...
   The **plain-English authoring rules** (T-221, §6) bind here too, and the
   `abstract` is where they bind first: it is the most-skimmed authored span in
   the base. Rules 1, 4, and 6 carry the most weight in a summary — a fact in an
   aside gets its own sentence, a citation sits at the end of the sentence it
   supports, and a longer plain sentence beats a shorter compressed one.
...
- **A structured source's summary states the schema reading and coverage
  (T-048).** For a spreadsheet, a large CSV, an export — meaning that lives in
  structure — the summary is where the schema is READ: what it tracks, at what
  grain, over what span, and where ("weekly MAU, one row per week, header row 3
  of sheet Metrics, covering January through December 2026"). Which row is the
  header and which columns matter is judgment — summary territory, never the
  deterministic aid's (the aid carries only mechanical facts: sheet names,
  dimensions, typed date/numeric ranges). The model-read pattern applied to
  structure: where the deterministic layer cannot reach the meaning, the summary
  carries the findable reading — a locator question ("which doc talks about MAU
  in July 2026?") is answered by a summary written to be found by those words.
...
**stale on a constitutional amendment**, not on every commit. **Before answering a code
question, know the footprint (`repo-coverage`, T-196).** The repo analog of the `connectors`
roster projects what the base actually *holds* about a repo — the constitution + surfaces, the
**evidence** captures (specific files/excerpts kept while answering earlier questions), the
synthesized **concepts**, and the constitution-named surfaces with **no dedicated capture**.
It classifies the question honestly (T-142, "a miss is not absence"): **known** (evidence or a
concept grounds it → answer, cite), **partial** (the mental model *locates* the subsystem but
no current source is held → say where it lives, then fetch and capture the live file — locator
`<repo>#<path>`, the T-196 convention — before grounding), or **unknown** (the repo is absent
from coverage → say so and offer to `capture-repo` it). A scoped query always returns one entry
(all-empty = unknown, never an error); the read never fetches, walks, or infers. A base that
knows a repo's *identity* does not thereby know its *internals* — this keeps that line honest.
...
to answer differently if the source doesn't cooperate. **A structured source is
answered from its bytes when the value is past the aid's reach (T-048):** where
the host has file tools, open the canonical source with a deterministic reader
and read the *targeted* cells (the summary's schema reading aims the read),
citing source and coordinates; a lookup or arithmetic roll-up is the
computed-answer class (checkable, reproducible, `extracted`-rung, datum-plus-rule
disclosed) — but *interpretation is not lookup*: judgments over the data ("why
did it drop") are analyst work, offered as a deliverable (ADR-0044) or labeled
synthesis (ADR-0051), never served as a cited fact; an op-only host disclosed
the aid's truncation honestly instead of guessing past it. A genuinely reusable answer
...
gate (T-153). **One deliberate exception — the labeled-synthesis rung
(ADR-0051):** a leg that is TRUE as honest paraphrase across sources but has
no quotable span anywhere in the bytes ("these sources develop the same
theme" is a claim about an *arrangement*) is filed as
`synthesis over [src-a] + [src-b]: <claim>` — labeled, never disguised as a
quote and never dropped for honesty's sake — and the doc's `derivation` is
stamped `synthesis` (weakest link, ADR-0011; `ask` discloses the rung). The
rung is for claims that *cannot* quote, never for claims the author would
rather not quote: span-verifiable legs still quote, the per-leg composition
self-check still applies, and an answer that is mostly arch is staged on the
candidates rail rather than written directly (ADR-0033). A synthesis label
always means the *adapter's* arch — operator knowledge never rides a
synthesis leg; it enters as a decision or a captured source and is cited
(ADR-0051's colleague boundary: the adapter is accountable for derived
documents, the operator for decisions and sources, and substance smuggled
across the boundary dies at the next regenerate, while substance filed
through its own door survives every one). An
...
**Coverage check before minting (T-206).** Wherever a new judgment-typed derived
doc is minted — synthesize's crystallize, map's manifest, a crystallized answer,
a deliverable landing — the adapter first queries the derived layer for existing
coverage of the same claim: the catalog query op by provenance (`cites=` each
grounding source, the strong overlap prior) and by full text (`retrieve` where
the op is absent). Hits are candidates to read, never verdicts (ADR-0027 §2).
The test for **same ground**: would a reader asking this question be routed to
both docs and get different answers (do the Covers/Answers facets overlap)?
Compatible overlap **folds** (ADR-0035) under the flow's existing consent; a
**conflict is never silently resolved** — the adapter presents both claims with
quoted spans and elicits one of four resolutions: supersede the old (ADR-0041,
replacement first) · drop the new · fold with the correction · keep both,
cross-cited, the disagreement labeled (legitimate when the sources genuinely
disagree). Ingest summaries are out of scope (one per source, L15 — they cannot
collide on ground). Lint verifies doc-to-source fidelity, never doc-to-doc
agreement; this habit is the check for the gap the invariants deliberately
leave to judgment (surfaced by a 2026-07-30 field incident: an OPEN question
authored beside the insight that had already resolved it).
...
**Availability is observed, never assumed (T-198).** This paragraph describes which
transports carry the tier; it is not a license to tell the user the tier is absent.
Before characterizing their environment, look: are the `odin_search`/`odin_retrieve`
tools bound in this session? Is `muninn_semantic.py` on the checkout being driven? If
you have not looked, do not assert - a degraded result already discloses itself via
`via`/`backend`, so make the call and let it speak.
...
**Close (T-216):** record the pass — `review-log clean|findings-surfaced [--scope <what>] [--detail <one hedged line>]` — the challenge-log precedent, and the **only** sanctioned way a review reaches `log.md` (an entry authored by hand is a fabricated operation; §5.4, L23). It records that the pass *ran*: no doc mark, no status field, no trust score, and deliberately **no finding count**, since a deterministic-looking tally would launder judgment as fact. `clean` states what the pass reported, never that the docs are warranted.

**Close, cost (T-152):** append the usage record — `usage-log review --scope <ids read> [--tokens <real count only>]`. Disposable operational state (ADR-0027), separate from the record above; the Core cannot otherwise see this verb's cost, so the record is the only measurement there is, and the `usage` report discloses when it's missing.

**Never:** edits a source or a derived doc (§I1/§I5 — it only reads and reports);
writes a durable "reviewed" mark; hand-writes its own log entry; presents its
judgment with false precision; or gates anything.

**Writes:** no knowledge. On the user's nod to a finding, the `regenerate` it
offers does the write; `review` itself only appends its own one-line history
through `review-log`, exactly as `challenge`/`map`/`drift-check` do.
...
**Intent:** perform the work the owner queued from another surface (the web
control plane, or any session) — the executor side of ADR-0048's two-plane
split. `queue/` is pre-cognition staging, the `inbox/` sibling: durable,
git-carried, owner-legible task files; never knowledge, never citable,
invisible to lint and retrieval.

**Consent model:** authoring the task **was** the consent act, scoped to
exactly the named operations on the named inputs. Processing needs no fresh
per-task consent — and a task authorizes nothing beyond what it names.

**Flow:** `queue-list --status pending` → oldest first → perform each task
with the ordinary verb flow for its type (every gate of that flow applies
unchanged). Faithful transforms the task names explicitly (capturing a named
inbox item) run directly; **authored cognition lands on the candidates rail**
(ADR-0045's lower-trust rule), never as a direct base write — review and
promotion stay human. Close each task with **`queue-restatus`** (the Core op,
T-209 — never a hand-edit): `done` with a note naming the outputs, `blocked`
with why, or `declined` with why. The op enforces the legal lifecycle
(`pending → done | declined | blocked | cancelled`; terminal states stay
terminal) and appends the task's own log line.

**Landing (git-backed bases).** A task is closed when its re-status reaches
the branch the other planes read, not when the op runs. Environments that push
to their own working branch (cloud sessions especially) strand the verdict —
and any knowledge the task produced — where no plane can see it. The executor
checks where its commit landed and, on a side branch, says so and offers the
merge; an unmerged branch is invisible to lint, staleness, and every other
honesty mechanism, because nothing can flag knowledge that never arrived.

**Never:** deletes a task (re-statused only — the append-only worklog
discipline); leaves an unperformable task silently pending; reports a task
complete when its verdict is stranded on an unmerged branch; cites a task or
grounds anything in one; exceeds the task's named scope.
...
user's word. **When a delegated sweep fetched the bytes, the handoff is
verified, never trusted (T-258):** the worker reports each fetched artifact's
path and sha256, and the re-capture re-hashes the artifact against that value
before using it — mismatch or missing means re-fetch, never capture; a
truncated artifact under a trusted id would otherwise carry full provenance
undetected. **And sources-current is never voiced as derived-docs-current**: a
clean sweep vouches only for the swept sources; derived docs may rest on
unswept sources (named from provenance in the sweep's blind-spots reporting)
or on repo-state facts no source carries, which no source sweep refreshes. **Unreachable is a transport fact, never a drift conclusion**:
...
**Plain-English authoring rules (T-221).** "Plainly" is a register, and the
register is specified rather than left to taste. These six rules bind every
authoring moment — the ingest summary, a synthesized insight, a map doc, a
deliverable, and the answer or digest spoken to the user — and each of those
moments points here. The goal is prose that reads plainly, not prose that is
shorter; plain writing is often longer, and that is the right trade.

The problem is density, not padding. A 2026-08-01 measurement over a
121-document base found about one parenthetical or semicolon per sentence, with
sentences averaging 22 to 29 words. The asides were not filler. They were the
honesty machinery itself: facts, hedges, and citations packed into parentheses
instead of getting sentences of their own.

1. If an aside carries a fact, give it its own sentence. Parentheses stay fine
   for asides carrying nothing a reader could need alone: a gloss, an id, a unit.
2. Write arrows as words. Arrows inside code, commands, and file paths stay.
3. Expand a term the first time it appears, or do not use it.
4. Put the citation at the end of the sentence it supports. This moves a
   citation; it never changes what one *is*. **Form** is untouched: a citation
   written into the base stays a **linked** citation (ADR-0038), and moving it
   to the sentence end is not licence to shorten it to a bare `[src-…]` (`relink`
   is the repair if a pass drifts). **Granularity** is untouched too, and
   ADR-0009 stands: each claim still cites the source that supports it, and a
   sentence carrying two claims from two sources becomes two sentences.
5. Emphasis is rare — at most one bolded thing per section. A label naming a
   list item or a facet is not emphasis.
6. Prefer a longer plain sentence to a shorter compressed one. Never drop a
   hedge, a label, or a citation to shorten.

Reader-vocabulary facet lines (ADR-0012) are keyword runs, not prose, and these
rules do not govern them. Structure is not in scope either: headers, a
strong-versus-weaker split, and a plain closing summary all stay. This is a
sentence-level contract, authored judgment rather than a Core guarantee, and no
linter enforces it. The eval-layer instrument
`scripts/adapter_eval/prose_density.py` measures whether a base's derived layer
actually holds the register.
...
**Re-deriving a summary is quote-gated; first authoring is not (T-223).** A
summary that already exists gets the T-153 containment check on re-derive, the
same one insights and the map products always get, and a mismatched span refuses
the write. Authoring a summary for the first time — in ingest, or when healing an
L15 gap — stays ungated. The line is **supervision, not doc type**: a re-derive
is a deliberate act a person asked for and is watching, whereas bulk ingest is
the lower-supervision path (ADR-0007) that one inexact quote should not be able
to block. **Line-wrap never decides whether the gate runs** (T-224): the check
reads the authored paragraph rather than the emitted lines, so a quote and the
citation vouching for it count as together even when a wrap separates them. A
source with **no text layer** verifies nothing and its spans are skipped rather
than failed — unverifiable is not fabricated, and the `model-read` stamp is what
...
candidates` (ADR-0033), `pending_tasks` (queued work awaiting an executor —
ADR-0048/T-209, feeds the process-the-queue offer), `captures_since_lint`
(feeds the synthesize offer),
