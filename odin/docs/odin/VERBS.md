...
Project the base's EXPLICIT relationship graph as neutral, renderer-agnostic JSON (odin.graph.v1). Every edge is a frontmatter fact — grounded_by (provenance, with per-rung staleness), see_also, member_of, evidence, superseded_by, excerpt_of (ADR-0039 anchors), from_repo (T-196 locators) — never an inference (no title-mention or similarity edges; the known-vs-inferred line stays sharp, ADR-0015). Read-only; `center`+`depth` restrict to one doc's neighborhood, `kinds`/`relations` filter. `cache_key` is a stat-sweep change detector for layout caches, not a content hash.

| Switch | | What it does |
|---|---|---|
| `--center <value>` | optional | Restrict to this doc's undirected neighborhood (unknown id errors). |
| `--depth <value>` | optional | Hops from center (default 1; ignored without center). |
| `--kind <value> (repeatable)` | optional | Keep only these node kinds (source \| derived \| project \| decision). |
| `--relation <value> (repeatable)` | optional | Keep only these edge relations (grounded_by, see_also, member_of, evidence, superseded_by, excerpt_of, from_repo). |
