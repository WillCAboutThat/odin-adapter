...
## The web adapter's exposure (ADR-0050)

odin-web is **LAN/VPN software by default**. An unauthenticated instance that
is reachable exposes more than reading: the control plane can **author queue
tasks that executor sessions later run** — the highest-leverage write in the
system — plus candidate promotion, lint recording, chat that spends a BYOK
key, and original source bytes at `/source/<id>/file`. The network boundary
(tailnet, VPN, firewall) is therefore the documented floor, not an
afterthought: if the instance is reachable only by you, that IS the personal
tier's designed posture.

Two optional layers sit on top (ADR-0050):

- **The single-token gate.** Claim a token on the first-run setup page (or
  pin `ODIN_WEB_TOKEN` in the environment — the env wins, and unlike the
  claimed file it cannot be lost with the base volume). Once set, every
  route needs the token — a login cookie for browsers, `Authorization:
  Bearer` for tools. Only the token's SHA-256 is stored; comparison is
  timing-safe. No accounts: replacing the token is re-claiming.
  **Caveat named in the ADR:** on plain LAN HTTP the token rides
  unencrypted, and a lost volume reopens a file-claimed instance
  (fresh-install equivalence) — pin the env var for any deployment that
  must stay locked.
- **Identity at the reverse proxy.** Several devices, several humans, or an
  internet-facing name → put authentication at the deployment boundary
  (Tailscale serve, Authelia, an OIDC proxy), where remote-mode git
  credentials already live. odin-web trusts the transport it is deployed
  behind rather than growing an identity system; per-user authorization is
  T-213's future, not this gate's.
