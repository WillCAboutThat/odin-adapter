...
def main():
    try:
        ...
        for r in roots:
            ...
            if errors:
                # T-245: a base another process is writing RIGHT NOW is mid-ingest,
                # not failed — report the honest state instead; the state file is
                # kept (blocks is non-empty), so the debt is re-checked next stop.
                if shared.write_in_flight(root):
                    blocks.append(shared.render_in_flight(
                        root, errors, "this session's writes"))
                else:
                    blocks.append(shared.render_errors(
                        root, errors, "this session's writes"))
