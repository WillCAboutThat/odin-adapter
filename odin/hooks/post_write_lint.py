...
def write_in_flight(root):
    """True iff another process holds this Muninn's advisory write lock RIGHT NOW
    (T-245). The Core holds `.odin/write.lock` (an OS lock, not a lockfile) only
    for the duration of each write op, so a held lock is definitionally a live
    writer — and lint findings against a mid-ingest base (L8/L15/L22-style) are
    its CORRECT intermediate state, not a failure; an observer acting on them
    would race the live session (the T-232 class). Non-blocking probe: never
    creates the lock file (a base never written has no writer), acquires and
    releases instantly when free. Best-effort — any infrastructure failure
    reports no-writer, which is exactly the pre-T-245 behavior."""
    try:
        p = Path(root) / ".odin" / "write.lock"
        if not p.exists():
            return False
        f = open(p, "a+")
        try:
...
def render_in_flight(root, errors, where):
    """The honest report for a base that is mid-write: what lint saw, why that is
    expected, and the posture (hands off; the debt stays owed)."""
    return (
        "odin: %s — lint sees %d error(s) after %s, but a live writer holds this "
        "base's write lock: this is a mid-ingest intermediate state (writes still "
        "settling), NOT a failure (T-245). Do not intervene or 'fix' the base "
        "while another session is writing it — that races the live writer "
        "(T-232). The lint debt stays recorded; it is re-checked once the "
        "writes settle." % (root, len(errors), where))
...
def main():
    ...
    if errors:
        if write_in_flight(root):
            print(render_in_flight(root, errors, verb), file=sys.stderr)
        else:
